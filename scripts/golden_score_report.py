...
def 마크다운(result: dict[str, Any]) -> str:
    lines = [
        ...
        "## 시트별 표\n\n",
        "| 시트 | 상태 | 골든 행수 | 출력 행수 | 매칭 수 | 엄격 | 완화 | 리콜 | 정밀도 | 값일치율 | 골든만 있는 값 | 출력만 있는 값 | 의미완화 |\n",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n",
    ]
    for sheet_name, sheet in result["시트별"].items():
        lines.append(_표행([sheet_name, sheet["상태"], sheet["골든행수"], sheet["출력행수"], sheet["매칭수"], sheet["엄격매칭수"], sheet["완화매칭수"], sheet["리콜"], sheet["정밀도"], sheet["값일치율"], sheet["골든만있는값"], sheet["출력만있는값"], sheet["의미완화매칭수"]]))
