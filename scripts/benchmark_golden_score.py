...
# ///
# ─── How to run ───
# .venv/bin/python scripts/benchmark_golden_score.py output.xlsx golden.xlsx --report-dir output --json
"""벤치마크 하네스 전용 채점기(v8 개정판 보존).

게이트·기준선 채점 축은 scripts/score_against_golden.py(v7-5)로 고정되어 있고, 이
일가족(benchmark_golden_*)은 utils/benchmark_evaluation.py의 고정 데이터셋 평가
(cell_accuracy 등 독립평가지표)만 담당한다. 두 채점기는 매칭·스키마가 달라 수치를
섞으면 안 된다."""
...
from scripts.benchmark_golden_contract import (  # noqa: E402
    경로라벨,
    데이터시트,
    페이지집합,
    수치시트,
...
from scripts.benchmark_golden_matching import (  # noqa: E402
    문자유사포함출처직렬화,
    시트점수,
    의미완화포함출처직렬화,
    없는골든요약,
...
from scripts.benchmark_golden_report import 마크다운  # noqa: E402
from scripts.benchmark_golden_workbook import 워크북읽기  # noqa: E402
