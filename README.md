...
이 프로젝트의 핵심 목표는 단순히 많이 뽑는 것이 아니라, **조용한 데이터 유실을 막고 사람이 검증할 수 있는 근거를 남기는 것**입니다. v4/v4.1이 안정성(부분 실패 허용, 배치 원장, quota 복구, 출처페이지)을 확보했다면, v5는 가이드라인 구조적 주입과 참조 사전으로 추출 충실도를 높이고, 행 단위 데이터상태·타깃 검수로 사람이 재확인할 지점을 파이프라인이 직접 지목하게 했습니다. v6는 **측정 체계**를 도입해 "얼마나 정확한가"를 골든셋 기준 숫자로 답할 수 있게 했습니다.
...
## v6에서 달라진 핵심 (2026-07-07)

v6의 방향은 하나입니다. **측정 없이는 개선도 없다** — 추출 정확도를 골든셋 기준 숫자로 만들고, 이후 모든 변경(임계 조정, 모델 교체, 병합 정책 완화)은 이 숫자의 개선으로만 정당화합니다. 기준 문서는 `docs/specs/프로젝트_마일스톤_로드맵.md`(M1·M2-1 완료)입니다.

### 1. 골든셋 대조 채점 하네스 (M1-2)

`scripts/score_against_golden.py`가 파이프라인 출력 xlsx와 골든셋 xlsx를 LLM 토큰 0으로 대조해 시트별 리콜/정밀도/값일치율과 **출처유형별(텍스트 유래 vs 시각 유래) 분해 리포트**를 산출합니다. 행 매칭은 organizer의 dedup 키 체계를 재사용하고, 수치는 0.5% 상대오차로 비교합니다. 골든셋 형식 계약은 `docs/specs/M1-2_골든셋대조하네스_구현명세서.md` §1 참조.

### 2. 시각 요소 인벤토리 리콜 감사 (M2-1)

`scripts/audit_visual_inventory.py`가 사람 확정 시각요소 목록과 `16_시각자료목록`을 대조해 요소별 유실 단계(이미지 추출 → triage → vision → 기록)와 임계 민감도 표를 산출합니다. 임계값은 바꾸지 않고 근거 숫자만 만듭니다.

### 3. 서울 골든셋과 첫 기준선 (`data/golden/`)

- `서울특별시_골든셋_v1_초벌.xlsx`: 독립 추출 3종 교차합의 + 원문 판정으로 만든 채점 1,760행(값불일치 77건 전량 원문 판정 완료). 부분 골든이므로 리콜은 유효, 정밀도는 골든이 차오를수록 유의미해집니다.
...
- v6 (2026-07-07): 골든셋 대조 채점 하네스(M1-2)·시각 요소 인벤토리 감사(M2-1) 도입, 서울 골든셋(1,760행)·시각 인벤토리(176요소) 구축, 첫 기준선 산출(텍스트 리콜 0.674·값일치율 0.829, 시각 codex 0.988 vs gemini flash-lite 0.707). 총 156개 테스트
